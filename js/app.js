...
function sanitizeUrl(url, fallback = '') {
    ...
    try {
        ...
        // Ignore malformed URLs and fall back safely.
    }
...
function appendGroundingSources(rawQuestions, groundingSources = []) {
    ...
    groundingSources.forEach(source => {
        ...
        if (uniqueSources.some(existing => existing.uri === source.uri)) return;
        uniqueSources.push({
    ...
    if (!uniqueSources.length) return questions;
...
function renderDiscussionQuestionsHtml(rawQuestions) {
    ...
    if (lines.length === 0) {
        return '<div class="text-center text-stone-400 py-6 italic"><p>No discussion guide available yet.</p><p class="text-xs mt-1">Generate one with AI or paste your own.</p></div>';
    }
    ...
    lines.forEach((line) => {
        if (line.startsWith('_Generated by') || line.startsWith('Generated by')) {
            attributionText = line.replace(/_/g, '');
        } else if (/^_?Sources?:/i.test(line)) {
            attributionText = line.replace(/_/g, '');
        } else {
            mainContent.push(line.replace(/^\d+[\.\)]\s*/, ''));
        }
    });

    let html = '<ol class="list-decimal pl-5 space-y-2">';
    ...
    if (attributionText) {
        html += `<p class="mt-8 text-sm text-stone-400 italic text-right border-t pt-2">${escapeHtml(attributionText)}</p>`;
    }

    return html;
}

function renderFormattedDescription(rawDescription) {
    const plainText = htmlToPlainText(rawDescription || 'No description available.');
    let desc = plainText.replace(/\r\n/g, '\n');

    desc = desc.replace(/\n{3,}/g, '\n\n');
    desc = desc.replace(/([.!?]["'])\s*[—-]{1,2}/g, '$1\n\n—');
    desc = desc.replace(/([—-]{1,2}[A-Z][^.!?<\n]{5,60}?)\s+(?=[A-Z][a-z])/g, '$1\n\n');
    desc = desc.replace(/\.\s+(From the (?:award-winning |bestselling |#1 )?author)/gi, '.\n\n$1');
    desc = desc.replace(/\.\s+(In (?:her|his|this|the) (?:new |latest |biggest )?)/gi, '.\n\n$1');
    desc = desc.replace(/\.\s+(When (?:the|she|he|they) )/gi, '.\n\n$1');
    desc = desc.replace(/\.\s+(Now,? (?:she|he|they|the) )/gi, '.\n\n$1');
    desc = desc.replace(/\.\s+(As (?:she|he|they|the) )/gi, '.\n\n$1');
    desc = desc.replace(/\.\s+(With (?:the|her|his) )/gi, '.\n\n$1');

    const paragraphs = desc.split(/\n{2,}/).map(p => p.trim()).filter(Boolean);
    const chunkedParagraphs = paragraphs.flatMap((paragraph) => {
...
function createBookCard(book) {
    ...
    const thumbnail = sanitizeUrl(info.imageLinks?.thumbnail, 'https://via.placeholder.com/128x192?text=No+Cover');
    const safeTitle = escapeHtml(info.title || 'Untitled');
    const safeAuthors = escapeHtml(info.authors ? info.authors.join(', ') : 'Unknown Author');
    const safeYear = escapeHtml(info.publishedDate ? info.publishedDate.substring(0, 4) : '');
    ...
    div.innerHTML = `
        <div class="book-frame-modern w-20 md:w-24 border-r border-stone-200 self-stretch">
            <img src="${thumbnail}" alt="${safeTitle}" class="book-cover-shadow w-full h-auto object-contain max-h-full" onerror="this.onerror=null; this.src='https://via.placeholder.com/128x192?text=No+Cover'">
        </div>

        <div class="p-3 md:p-4 flex flex-col justify-center min-w-0 flex-grow">
            <h3 class="font-serif font-bold text-base text-stone-800 leading-tight mb-1 truncate" title="${safeTitle}">${safeTitle}</h3>
            <p class="text-xs text-stone-600 mb-1 truncate">${safeAuthors}</p>
            <p class="text-[10px] text-stone-400 mb-2">${safeYear}</p>
...
function openModal(book, savedData = null) {
    ...
    modalDescription.innerHTML = renderFormattedDescription(info.description);
    ...
    if (isSaved) {
        ...
        if (savedData) {
            ...
            if (canOpenDiscussionGuide) {
                ...
                // Hide old right column button
                if (btnOpenGuide) btnOpenGuide.parentElement.classList.add('hidden');
            } else {
                // Hide both buttons for non-Scheduled books
                if (discussionGuideLeft) discussionGuideLeft.classList.add('hidden');
                if (btnOpenGuide) btnOpenGuide.parentElement.classList.add('hidden');
            }
        } else {
        ...
        if (discussionGuideLeft) {
            discussionGuideLeft.classList.add('hidden');
        }
...
async function removeTagFromCard(e, bookId, tag) {
    ...
    const decodedTag = typeof tag === 'string' ? decodeURIComponent(tag) : tag;
    if (!confirm(`Remove tag "${decodedTag}"?`)) return;
...
function renderDashboard() {
    ...
    const nextTitle = escapeHtml(nextInfo.title || 'Untitled');
    const nextAuthors = escapeHtml(nextInfo.authors ? nextInfo.authors.join(', ') : 'Unknown Author');
    const nextHost = nextBook.host_name ? escapeHtml(nextBook.host_name) : '';
    const nextThumbnail = sanitizeUrl(nextInfo.imageLinks?.thumbnail?.replace('http:', 'https:'), 'https://via.placeholder.com/300x450?text=No+Cover');
    ...
    dashboardHero.innerHTML = `
        ...
        <div class="flex-grow p-4 md:p-6 flex flex-col justify-center min-w-0">
            ...
            <h2 class="font-serif text-xl md:text-3xl font-bold text-stone-900 mb-1 leading-tight line-clamp-2">${nextTitle}</h2>
            <p class="text-xs md:text-sm text-stone-500 mb-3 md:mb-5 font-medium truncate">by ${nextAuthors}</p>
            ...
            <div class="flex flex-wrap gap-x-6 gap-y-2 mb-4 md:mb-6 pt-3 md:pt-5 border-t border-stone-100">
                ...
                ${nextBook.host_name ? `
                    <div>
                        ...
                        <p class="text-sm md:text-base font-bold text-stone-800">${nextHost}</p>
                    </div>` : ''}
            </div>
    ...
    if (laterBooks.length > 0) {
        ...
        dashboardUpcomingList.innerHTML = laterBooks.map(book => {
            const info = book.google_data.volumeInfo;
            const safeTitle = escapeHtml(info.title || 'Untitled');
            const safeAuthor = escapeHtml(info.authors ? info.authors[0] : 'Unknown');
            const safeHost = book.host_name ? escapeHtml(book.host_name) : '';
            ...
            const thumbnail = sanitizeUrl(info.imageLinks?.thumbnail?.replace('http:', 'https:'), 'https://via.placeholder.com/128x196?text=No+Cover');
            ...
            const hostHtml = (() => {
                if (book.host_name) {
                    return `<div class="mt-1 text-center w-full"><p class="text-[10px] text-stone-500 truncate px-1">Host: ${safeHost}</p></div>`;
                } else {
        ...
        <div class="card-modern flex items-center fade-in group relative">
                    <div class="book-frame-modern w-28 border-r border-stone-200 cursor-pointer self-stretch flex flex-col items-center py-2 bg-stone-50/30" onclick="openModal(allSavedBooks.find(b => b.id === ${book.id}).google_data, allSavedBooks.find(b => b.id === ${book.id}))">
                        <img src="${thumbnail}" alt="${safeTitle}" class="book-cover-shadow w-20 h-auto object-contain shadow-md" onerror="this.onerror=null; this.src='https://via.placeholder.com/128x196?text=No+Cover'">
                        ${ratingHtml}
                        ${hostHtml}
                    </div>
        ...
        <div class="px-5 py-4 flex flex-col justify-center min-w-0 flex-grow">
            <div onclick="openModal(allSavedBooks.find(b => b.id === ${book.id}).google_data, allSavedBooks.find(b => b.id === ${book.id}))" class="cursor-pointer pt-1">
                ...
                <h3 class="font-bold text-stone-900 text-sm leading-snug mb-1 truncate">${safeTitle}</h3>
...
function renderSavedBooks(books) {
    ...
    books.forEach(row => {
        ...
        const thumbnail = sanitizeUrl(info.imageLinks?.thumbnail, 'https://via.placeholder.com/128x192?text=No+Cover');
        const safeTitle = escapeHtml(row.title || info.title || 'Untitled');
        const safeAuthors = escapeHtml(info.authors ? info.authors.join(', ') : 'Unknown');
        const status = row.status || 'Saved';
        const statusClass = getStatusColor(status);
        const rating = row.rating ? `★ ${row.rating} ` : '-';
        const safeDate = escapeHtml(row.target_date || '-');
        // Derive club year from target_date
        const clubYear = row.target_date ? row.target_date.split('-')[0] : '-';
        const safeHost = escapeHtml(row.host_name || '-');
        ...
        const tagsHtml = visibleTags.length > 0
            ? `<div class="mt-2 flex flex-wrap gap-1"> ${visibleTags.map(t => `
                <span class="inline-flex items-center text-[10px] px-1.5 py-0.5 rounded font-medium ${getTagColor(t)} group/tag">
                    ${escapeHtml(t)}
                    <button onclick="removeTagFromCard(event, '${row.id}', '${encodeURIComponent(t)}')" class="ml-1 hidden group-hover/tag:inline-flex hover:text-black/50 items-center justify-center">
        ...
        const hostHtml = (() => {
            ...
            if (row.host_name) {
                return `<div class="mt-1 text-center w-full"><p class="text-[10px] text-stone-500 truncate px-1">Host: ${escapeHtml(row.host_name)}</p></div>`;
        ...
        card.innerHTML = `
                <div class="book-frame-modern w-28 border-r border-stone-200 self-stretch flex flex-col items-center py-2 bg-stone-50/30">
                    <img src="${thumbnail}" alt="${safeTitle}" class="book-cover-shadow w-20 h-auto object-contain shadow-md" onerror="this.onerror=null; this.src='https://via.placeholder.com/128x192?text=No+Cover'">
                    ${ratingHtml}
                    ${hostHtml}
                </div>
        ...
        <div class="p-3 md:p-4 flex flex-col justify-center min-w-0 flex-grow">
            ...
            <p class="text-xs text-stone-600 mb-2 truncate">${safeAuthors}</p>
    ...
    `;
        ...
        <td class="px-6 py-4 whitespace-nowrap" data-label="Book">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <div class="text-sm font-medium text-stone-900 truncate max-w-[200px]" title="${safeTitle}">${safeTitle}</div>
                    <div class="text-sm text-stone-500 truncate max-w-[150px]">${safeAuthors}</div>
                </div>
            </div>
            </td>
...
async function getGoodreadsRating(isbn, title, author) {
    ...
    try {
        for (const searchQuery of searchQueries) {
            const response = await fetchWithSession(`${GOODREADS_PROXY_URL}?q=${encodeURIComponent(searchQuery)}`);
            const data = await response.json().catch(() => ({}));
...
const OFFICIAL_DISCUSSION_GUIDES = {
    'The Secret Book Society': {
        sourceName: 'Madeline Martin official book club kit',
        sourceUrl: 'https://madelinemartin.com/wp-content/uploads/2025/09/THE-SECRET-BOOK-SOCIETY-Book-Club-Kit.pdf',
        mirroredSourceName: 'Bookclubs discussion guide',
        mirroredSourceUrl: 'https://bookclubs.com/discussion-guides/the-secret-book-society',
        questions: [
...
function getDiscussionQuestions(book) {
    ...
    const officialGuide = OFFICIAL_DISCUSSION_GUIDES[title];
    if (officialGuide) {
        return formatOfficialDiscussionGuide(officialGuide);
    }
...
function formatOfficialDiscussionGuide(guide) {
    const questions = guide.questions
        .map((question, index) => `${index + 1}. ${question}`)
        .join('\n\n');
    return `${questions}\n\n_Source: ${guide.sourceName} (${guide.sourceUrl})_`;
}
...
function lintDiscussionGuideOutput(rawQuestions, title = '') {
    const text = htmlToPlainText(rawQuestions || '');
    const lower = text.toLowerCase();
    const issues = [];
    const verifiedContext = VERIFIED_GUIDE_CONTEXT[title];
    ...
    const candidateLines = text
        ...
        .filter(Boolean)
        .filter(line => !/^_?(generated by|source:)/i.test(line));
    const questions = candidateLines.filter(line => line.endsWith('?'));

    if (candidateLines.length !== questions.length) {
        issues.push('Return only numbered questions, with no introduction, explanation, heading, or closing text.');
    }

    if (questions.length < 10 || questions.length > 15) {
        issues.push('Return 10 to 15 questions, using only as many as the source material can support well.');
    }

    const multiPartCount = questions.filter(question => {
        const questionMarkCount = (question.match(/\?/g) || []).length;
        const longCompound = question.length > 240 && /\b(and|but)\s+(did|do|does|how|what|when|where|which|who|why|would|could|can)\b/i.test(question);
        return questionMarkCount > 2 || longCompound;
    }).length;
    ...
    if (verifiedContext?.keywords?.length) {
        const anchoredCount = questions.filter(question => {
            ...
            return verifiedContext.keywords.some(keyword => lowerQuestion.includes(keyword.toLowerCase()));
        }).length;

        if (anchoredCount < Math.min(6, questions.length - 2)) {
            issues.push('Use more concrete book anchors from the verified context instead of broad generic prompts.');
        }
    }
...
async function generateDiscussionQuestionsAI() {
    ...
    try {
        ...
        if (officialGuide) {
            const questions = formatOfficialDiscussionGuide(officialGuide);
            currentDiscussionBook.discussion_questions = questions;

            if (currentDiscussionBook.id) {
                let officialGuideUpdateQuery = supabase
                    .from('book_club_list')
                    .update({ discussion_questions: questions })
                    .eq('id', currentDiscussionBook.id);

                officialGuideUpdateQuery = applyActiveClubFilter(officialGuideUpdateQuery);

                await officialGuideUpdateQuery;
            }
...
Search first for official or citeable book-club questions from the author, publisher, book-club kit, or reputable book-club guide pages.

Decision rule:
- If official author/publisher/book-club-kit questions exist, use those questions directly and cite the source.
...
- Ask readers to judge a character's action, motive, or responsibility.
- Ask whether their opinion changed as they learned more.
- Connect specific book details to modern life or the group's own experience.
- Ask what the reader would have done in the same situation.
- Use a concrete object, rumor, relationship, or conflict as the doorway into discussion.
- Invite disagreement, mixed feelings, and personal connection.
...
- Prefer natural openings such as "How did your opinion of...", "Do you think...", "Would you have...", "Which character...", "What modern parallels...", "What books have...", "Where did you see...", or "What line stayed with you..."
...
Style rules:
- Do not include an introduction such as "Here are..." or "Discussion questions for..."
...
- End with one Source line naming the source used; if no source was found, end with "Source: AI-generated from available book metadata."

Return ONLY a numbered list of 10 to 15 questions plus one Source line.`;
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
            ...
            qualityIssues = lintDiscussionGuideOutput(questions, title);
        ...
        if (data.modelUsed) {
            ...
            questions += `\n\n_Generated by ${data.modelUsed} on ${today}_`;
        }
...
async function saveDiscussionQuestions() {
    ...
    try {
        ...
        const viewEl = document.getElementById('discussion-content-view');
        viewEl.innerHTML = renderDiscussionQuestionsHtml(newQuestions);
...
// --- Discussion Guide 2.0 UI Helper ---
function renderDiscussionGuideUI(book) {
    const viewEl = document.getElementById('discussion-content-view');
    const editEl = document.getElementById('discussion-content-edit');
    const btnGenerate = document.getElementById('btn-generate-ai');
    const btnEdit = document.getElementById('btn-edit-questions');
    // btnCopy removed
    ...
    // Reset Subtitle to default initially
    if (subtitleEl) subtitleEl.textContent = 'Book Club Companion';
...
function printDiscussionGuide() {
    ...
    printWindow.document.write(`
        ...
        // Filter out empty lines AND the AI attribution line
        return trimmed.length > 0 &&
            !trimmed.startsWith('_Generated by') &&
