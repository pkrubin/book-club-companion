...
function renderDiscussionGuideUI(book) {
    ...
    if (questions && questions.length > 0) {
        // Populated State
        // Convert to HTML list if it's plain text
        let htmlContent = '';
        if (!questions.trim().startsWith('<')) {
            const lines = questions.split(/\n+/).filter(line => line.trim().length > 0);


            // Check for attribution (last line usually)
            const mainContent = [];
            let attributionText = '';
            ...
            htmlContent = '<ol class="list-decimal pl-5 space-y-2">';
            mainContent.forEach(item => {
                // Remove existing numbering if present (e.g. "1. ") to avoid double numbering
                const cleanItem = item.replace(/^\d+[\.\)]\s*/, '');
                htmlContent += `<li>${cleanItem}</li>`;
            });
            htmlContent += '</ol>';

            // Append attribution outside the list (Restored)
            if (attributionText) {
                htmlContent += `<p class="mt-8 text-sm text-stone-400 italic text-right border-t pt-2">${attributionText}</p>`;
            }
        } else {
        ...
        // Empty State
        viewEl.innerHTML = '<div class="text-center text-stone-400 py-6 italic"><p>No discussion guide available yet.</p><p class="text-xs mt-1">Generate one with AI or paste your own.</p></div>';
        if (btnGenerate) {
